...
// TODO #1 - DomContentLoaded 모든 HTML 문서가 로드된 상태 - 그래야 DOM element에 접근 가능
window.addEventListener("DOMContentLoaded", function () {
    ...
    // TODO #3 - loginForm submit(전송) 이벤트 등록 (submit 이벤트는 로그인 버튼 클릭시 발생)
    // 단 로그인 버튼의 button type = 'submit' 이어야 함!! type = 'button'일 때 동작 X
...
function getCartItems(userId, cartId, displayCartItems) {
    const xhr = new XMLHttpRequest();
    const url = SERVER_URL + "/ap i/nhnmart/shopping-cart/" + cartId;
